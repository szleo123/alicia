...
Usage:
    ...
    python grasp_execution.py [options]
"""
...
class GraspExecutionNode(Node):
    """
    ROS 2 Node for grasp execution using MoveIt 2.
    """
    
    # Default HOME position (joint angles in radians)
    # HOME_POSITION = [0.0, 0.8199, -0.1570, 0.0, -0.7676, 0.0]
    HOME_POSITION = [0.0, 0.8722, -0.0174, 0.0, -1.1164, 0.0]
    # HOME_POSITION = [0.0, 0.1089, 0.6703, 0.0, -1.4174, 0.0]
    
    # Gripper configuration
    GRIPPER_DEPTH = 0.13  # Distance from gripper base center to fingertip (m)
    GRIPPER_OPEN = [0.0]  # Gripper open position
    GRIPPER_CLOSED = [0.024]  # Gripper closed position
    
    # Movement parameters
    PRE_GRASP_DISTANCE = 0.06  # Pre-grasp approach distance (m)
    ...
    # Positive values move the grasp in the positive direction of each axis
    OFFSET_X = -0.0   # X offset in base frame (towards front of robot)
    OFFSET_Y = -0.0  # Y offset in base frame (towards left of robot) - compensate for +5cm error
    OFFSET_Z = 0.0   # Z offset in base frame (upward)
    ...
    def __init__(self, args):
        ...
        # Uses TF2 to get full transform chain: camera_depth_optical_frame -> base_link
        # This properly handles optical frame conventions from depth cameras
        ...
        self.workflow_state = "init"  # init -> wait_grasps -> processing -> executing
        self.selected_grasp = None
        self.ik_solutions = []
        self.current_grasp_index = 0
        
        # Timer for workflow execution (matches cube_sorting.py pattern)
        self.workflow_timer = self.create_timer(0.5, self._workflow_tick)
        
        self.get_logger().info("Grasp Execution Node initialized")
    ...
    def _publish_hand_eye_tf(self):
        ...
        try:
            ...
            # This accounts for the difference between camera_link and camera_*_optical_frame
            m_internal = np.array([
                [ 0.0, -0.0,  1.0, 0.002],
                [-1.0, -0.0, -0.0, -0.014],
                [ 0.0, -1.0,  0.0, 0.000],
    ...
    def _init_ros_interfaces(self):
        ...
        # 高亮索引发布者 - grasp_generation.py 订阅此 topic 来高亮对应的夹爪
        # 发布 -1 表示清除高亮
        ...
        # Wait for action servers (critical for reliable operation)
    ...
    def get_gripper_pose_in_base(self) -> Optional[np.ndarray]:
        """Get current gripper pose in base_link frame from TF."""
        try:
            ...
            self.get_logger().warn(f"Could not get gripper pose from TF: {e}")
            return None
    ...
    def transform_grasp_to_base(self, grasp_cam: np.ndarray) -> np.ndarray:
        ...
        This method uses TF2 to get the full transform chain from 
        camera_depth_optical_frame to base_link. This properly handles:
        - The optical frame convention used by depth cameras
        - The hand-eye calibration (which is published to TF by the calibration launch)
        
        Also applies:
        1. Gripper convention alignment (GraspGen X=open -> actual Y=open)
        2. Gripper offset correction (base center to fingertip)
        
        Args:
            grasp_cam: Grasp pose in camera optical frame (4x4)
            
        Returns:
            grasp_base: Grasp pose in base_link frame (4x4), or None if transform failed
        """
        try:
            # Get transform from camera optical frame to base using TF2
            # This automatically handles the full transform chain including:
            # camera_depth_optical_frame -> camera_link -> gripper_center -> ... -> base_link
            transform = self.tf_buffer.lookup_transform(
                self.BASE_FRAME, self.CAMERA_FRAME,
                rclpy.time.Time(), timeout=rclpy.duration.Duration(seconds=1.0))
            ...
            # Extract transform as 4x4 matrix
            t = transform.transform.translation
            ...
            # GraspGen: X = finger opening direction, Z = approach
            # Actual gripper: Y = finger opening direction, Z = approach
            # Need to rotate -90 degrees around Z axis
            R_align = R.from_euler('z', -90, degrees=True).as_matrix()
            grasp_base[:3, :3] = grasp_base[:3, :3] @ R_align
            
            # Gripper offset correction:
            # GraspGen outputs gripper base center pose
            # If calibration is for fingertip, subtract offset along gripper Z axis
            if self.GRIPPER_DEPTH > 0:
                approach_dir = grasp_base[:3, 2]  # Z-axis of grasp frame
                offset = approach_dir * self.GRIPPER_DEPTH
                grasp_base[:3, 3] += offset
            
            # Apply coordinate offset correction (for fine-tuning)
            # This compensates for systematic errors in calibration or camera alignment
            ...
            self.get_logger().error(f'  Frames: {self.CAMERA_FRAME} -> {self.BASE_FRAME}')
            self.get_logger().error('  Make sure the camera driver is running and TF is published')
            return None
    ...
    def filter_grasp_poses(self, grasps: List[np.ndarray], 
        ...
        Filters:
        1. Z height (not too low)
        2. Approach direction not pointing up (no bottom-up grasps)
        
        Args:
            grasps: List of grasp poses (4x4 matrices) in base frame
        ...
        for i, (grasp, conf) in enumerate(zip(grasps, confidences)):
            ...
            approach_z = grasp[2, 2]  # Z component of approach vector
            
            # Filter criteria
            if z < self.args.min_z:
                self.get_logger().debug(f"Grasp {i}: filtered (z={z:.3f} < {self.args.min_z})")
                continue
            
            if approach_z > 0:  # Approach vector pointing up = bottom-up grasp
                self.get_logger().debug(f"Grasp {i}: filtered (bottom-up approach)")
                continue
    ...
    def compute_ik(self, position: np.ndarray, orientation: np.ndarray, 
        ...
        Args:
            ...
            timeout_sec: Maximum time to wait for IK solution
        ...
        if response.error_code.val != MoveItErrorCodes.SUCCESS:
            return None  # IK failed - skip silently
    ...
    def move_arm_with_moveit(self, joint_positions: List[float]) -> bool:
        """
        Move arm using MoveIt plan + execute (more reliable).
    ...
    def move_to_pose(self, pose: np.ndarray, duration_sec: float = 2.0) -> bool:
        ...
        Args:
            ...
            duration_sec: Movement duration
    ...
    def compute_pre_grasp_pose(self, grasp_pose: np.ndarray) -> np.ndarray:
        ...
        approach_dir = grasp_pose[:3, 2]  # Z-axis is approach direction
    ...
    def highlight_grasp_in_meshcat(self, orig_idx: int):
        ...
        Args:
            orig_idx: 原始夹爪索引（在 grasp_poses 中的索引）
        """
        msg = Int32()
        msg.data = orig_idx
    ...
    def execute_grasp(self, grasp_pose: np.ndarray) -> bool:
        ...
        Args:
            grasp_pose: Target grasp pose in base frame (4x4)
    ...
    def print_pose_info(self, idx: int, grasp: np.ndarray, conf: float):
        ...
        print("\n" + "=" * 60)
        print(f"Grasp Candidate #{idx}")
        print("=" * 60)
        print(f"Confidence: {conf:.3f}")
        print(f"Position (m): x={position[0]:.4f}, y={position[1]:.4f}, z={position[2]:.4f}")
        print(f"Orientation (deg): roll={euler[0]:.2f}, pitch={euler[1]:.2f}, yaw={euler[2]:.2f}")
        print(f"Quaternion: x={quat[0]:.4f}, y={quat[1]:.4f}, z={quat[2]:.4f}, w={quat[3]:.4f}")
        print("=" * 60)
    
    def _workflow_tick(self):
        """
        Timer-based workflow execution (matches cube_sorting.py pattern).
        This is called periodically by the timer, allowing the executor to
        process callbacks between ticks.
        """
        if self.workflow_state == "init":
            # Move to HOME position first
            self.get_logger().info("=" * 60)
            self.get_logger().info("Grasp Execution Node Running")
            self.get_logger().info("=" * 60)
            self.get_logger().info("Moving to HOME position...")
            ...
            if self.move_to_home():
                ...
                # Stay in init state, will retry on next tick
        
        elif self.workflow_state == "wait_grasps":
            # Check for new grasps
            with self.grasp_lock:
            ...
            # Transform all grasps to base frame
            grasps_base = []
            ...
            # Pre-sort by: top-down approach (Z-axis pointing down), high score, low joint change
            def compute_presort_score(item):
                orig_idx, grasp, conf = item
                # Top-down score: prefer Z-axis of gripper pointing downward (negative z)
                approach_dir = grasp[:3, 2]  # Gripper Z-axis (approach direction)
                topdown_score = -approach_dir[2]  # Higher if pointing down
                # Joint distance estimate (use position distance as proxy before IK)
                pos = grasp[:3, 3]
                pos_dist = np.linalg.norm(pos - np.array([0.3, 0, 0.3]))  # Distance from home-ish
                # Combined pre-sort: topdown + confidence - position penalty
            ...
            # User input is handled in _prompt_user_selection
            # This state just waits for the input thread to complete
            pass
        
        elif self.workflow_state == "executing":
            # Execution is handled synchronously in _execute_selected_grasp
            pass
    ...
    def _prompt_user_selection_single(self, init_orig_idx, init_grasp, init_conf, init_joints, init_score):
        ...
        If user skips, continues searching for next valid IK.
        """
        import threading
        
        def user_input_thread():
            # Store the current grasp info
            current_grasp = (init_orig_idx, init_grasp, init_conf, init_joints, init_score)
            
            while True:
                if current_grasp is None:
                    # Need to find more IK solutions
                ...
                current_grasp = None  # Will need to find next if user skips
        ...
        # Run in a separate thread to not block ROS callbacks
    ...
    def _find_next_ik_solution(self) -> bool:
        ...
        # Get indices we've already tried
        ...
        for orig_idx, grasp, conf in grasps_base:
            ...
            if grasp[2, 3] < z_min:
                continue
            approach_dir = grasp[:3, 2]
            if approach_dir[2] > 0.3:  # Skip upward grasps
                continue
...
def main():
    parser = argparse.ArgumentParser(description='Grasp Execution Node')
    ...
    # (matches cube_sorting.py pattern - executor controlled in main, not in run())
