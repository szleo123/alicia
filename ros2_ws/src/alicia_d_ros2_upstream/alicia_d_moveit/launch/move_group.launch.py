"""Move group launch file for Alicia-D with version and gripper type selection."""
...
def launch_setup(context, *args, **kwargs):
    """Setup move_group launch with versioned config."""
    ...
    print(f'\033[1;32m[INFO] Starting move_group with robot version: {robot_version}, gripper type: {gripper_type}\033[0m')
    
    # Get versioned MoveIt config
    moveit_config = get_versioned_moveit_config(robot_version, gripper_type)
    
    # Generate standard move_group launch
    move_group_nodes = generate_move_group_launch(moveit_config)
    
    return move_group_nodes.entities
...
def generate_launch_description():
    ...
    return LaunchDescription([
        ...
        DeclareLaunchArgument(
            'info',
            default_value='true',
            description='Print info messages'
        ),
        DeclareLaunchArgument(
            'debug',
            default_value='false',
