...
def solve_geometric_corrections(chain, samples, param_names, base_corrections, prior_std_m):
    if len(samples) < 10:
        raise ValueError("Need at least 10 samples for reduced geometric calibration.")

    def unpack(x):
        corrections = build_correction_map(param_names, x[:len(param_names)], base_corrections)
        pivot = x[len(param_names):len(param_names) + 3]
        return corrections, pivot

    def residual_vector(x):
        corrections, pivot = unpack(x)
        residuals = []
    ...
    corrections, pivot = unpack(result.x)
    sample_errors = []
    for sample in samples:
        T = chain.fk(sample["joint_positions"], corrections)
        sample_errors.append(float(np.linalg.norm(T[:3, 3] - pivot)))
    errors = np.array(sample_errors, dtype=float)
...
class GeometricPivotCalibrator(Node):
    def __init__(self, args):
        super().__init__("geometric_pivot_calibrator")
        self.args = args
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Fit reduced geometric corrections from pivot contact samples."
    )
    parser.add_argument(
        "--urdf-file",
    ...
    parser.add_argument(
        "--samples-output",
        default=os.path.expanduser(f"~/alicia/calibration/kinematic/{run_stamp}/geometric_pivot_samples.yaml"),
...
def print_instructions(args):
    print()
    print("Reduced geometric pivot calibration")
    print("----------------------------------")
    ...
    print("4. Solve for a reduced set of URDF joint-origin corrections.")
    print()
    print("This first-pass solver fits only translation components of selected joint origins.")
    print("It is intentionally conservative and regularized.")
    print()
    if args.auto:
...
def main():
    ...
    rclpy.init()
    node = GeometricPivotCalibrator(args)
    ...
    try:
        ...
        result = solve_geometric_corrections(
            chain,
            node.samples,
            args.fit_params,
        ...
        node.get_logger().info("Solved reduced geometric calibration")
        ...
        if result["rmse"] > 0.01:
            node.get_logger().warn(
                ...
                "or the contact samples are not repeatable on a single physical point."
            )
